...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const getTranslationKey = (path: string) => {
    return (
      {
        press: "navbar_menu6_child1",
        blog: "navbar_menu6_child2",
        csr: "navbar_menu6_child3",
      }[path] || path
    );
  };
  ...
  const getBannerKey = (path: string) => {
    return (
      {
        press: "/images/media/banner-1.webp",
        blog: "/images/media/banner-2.webp",
        csr: "/images/media/banner-3.webp",
      }[path] || path
    );
  };
